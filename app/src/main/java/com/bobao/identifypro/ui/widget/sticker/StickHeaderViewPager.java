...
import com.bobao.identifypro.ui.widget.sticker.scroll.ScrollFragment;
import com.bobao.identifypro.ui.widget.sticker.scroll.ScrollHolder;
...
public class StickHeaderViewPager extends StickHeaderLayout implements ScrollHolder, ViewPager.OnPageChangeListener {
    ...
    //给用户提供的滚动监听,默认滚动整个headerView的高
    public interface OnStickerScrollListener {
        void onStickerScroll(View headView, int scrollY, int minHeaderTranslation);
    }
    private OnStickerScrollListener mStickerScrollListener;

    public void setOnStickerScrollListener(OnStickerScrollListener listener) {
        mStickerScrollListener = listener;
    }

    private void scrollHeader(int scrollY) {
        float translationY = Math.max(-scrollY, mMinHeaderTranslation);
        if(mStickerScrollListener == null) {
            mStickheader.setTranslationY(translationY);
        } else {
            mStickerScrollListener.onStickerScroll(mStickheader, scrollY, mMinHeaderTranslation);
        }
    }

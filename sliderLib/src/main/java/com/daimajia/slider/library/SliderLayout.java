 ...
 * {@link com.daimajia.slider.library.Tricks.ViewPagerEx} . There is some
 * properties you can set in XML: indicator_visibility visible invisible
 * indicator_shape oval rect indicator_selected_color indicator_unselected_color
 * indicator_selected_drawable indicator_unselected_drawable pager_animation
 * Default Accordion Background2Foreground CubeIn DepthPage Fade FlipHorizontal
 * FlipPage Foreground2Background RotateDown RotateUp Stack Tablet ZoomIn
 * ZoomOutSlide ZoomOut pager_animation_span
 */
public class SliderLayout extends RelativeLayout {
    ...
    public SliderLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        attributes.recycle();
        PresetIndicators presetIndicators = PresetIndicators.Center_Bottom;
        setPresetIndicator(presetIndicators);
    ...
    public List<BaseSliderView> getSliderViews() {
        if (mSliderAdapter != null) {
            return mSliderAdapter.getImageContents();
        }
        return null;
    }
    ...
    public void setSwipeView(View v){
        if(mIndicator != null){
            mIndicator.setSwipeView(v);
        }
    }
    ...
    public <T extends BaseSliderView> void addAllSlider(List<T> imageContents) {
        mSliderAdapter.addAllSlider(imageContents);
    }
    ...
    private android.os.Handler mh = new android.os.Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            try {
                int currectPostion = mViewPager.getCurrentItem();
                int itemCount = getRealAdapter().getCount();
                int virtualNextPosition = currectPostion + 1;
                int nextItemPosition = virtualNextPosition % itemCount;
                if (!mSliderAdapter.getImageContents().get(nextItemPosition).isComplate()) {
                    return;
                }
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public void startAutoCycle() {
        startAutoCycle(slideInterval, mSliderDuration, mAutoRecover);
    }
    ...
    /**
     * set the duration between two slider changes. the interval and duration
     * value must both >= 500
     * 
     * @param duration
     */
    public void setDuration(long interval, long duration) {
        if (interval < 500) {
            interval = 500;
        }
